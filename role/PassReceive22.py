...
class Pass(Main):
    ...
    def execute_fine_receive(self):
        ...
        if not vicinity_points(self.kub.get_pos(),self.kub.state.ballPos,BOT_BALL_THRESH):
            self.pass_failed = True
            return
...
class Receive(Main):
    ...
    def __init__(self,max_vel = MAX_BOT_SPEED/1.2,min_vel = MIN_BOT_SPEED*1.2):
        ...
        self.add_state(Receive.State.setup_receive,Main.State.Receive)
        self.add_state(Receive.State.in_2_alpha,Main.State.Receive)
        self.add_state(Receive.State.outside_circles,Main.State.Receive)
        self.add_state(Receive.State.inside_circles,Main.State.Receive)
        self.add_state(Receive.State.move_on_circle, Main.State.Receive)
    ...
    def at_target_point(self):
        kub_pos = Vector2D(self.kub.get_pos().x, self.kub.get_pos().y)
        ball_pos = Vector2D(self.kub.state.ballPos)
        if kub_pos.dist(ball_pos)<=BOT_BALL_THRESH*2:
            return True
        return False
